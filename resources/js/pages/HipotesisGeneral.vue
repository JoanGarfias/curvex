...
// =========================================================
//  1. LÓGICA TABLA 2.2 (PRUEBA Z)
// =========================================================
const z_modoSeleccionado = ref<number | null>(null);
const z_mostrarResultados = ref(false);
const z_cargando = ref(false);
// Formulario Z
const z_promedio = ref(''); const z_u0 = ref(''); const z_desviacion = ref(''); const z_confiabilidad = ref(''); const z_cantidad = ref('');
const z_promedio1 = ref(''); const z_desviacion1 = ref(''); const z_cantidad1 = ref('');
const z_promedio2 = ref(''); const z_desviacion2 = ref(''); const z_cantidad2 = ref('');
// Resultados Z
const z_z0 = ref<number | null>(null); const z_za = ref<number | null>(null); const z_veredicto = ref('');
...
const z_formularioValido = computed(() => {
  ...
  if (z_esDoble.value) return z_promedio1.value && z_desviacion1.value && z_cantidad1.value && z_promedio2.value && z_desviacion2.value && z_cantidad2.value && z_confiabilidad.value;
  else return z_promedio.value && z_u0.value && z_desviacion.value && z_confiabilidad.value && z_cantidad.value;
});
...
const z_limpiarFormulario = () => {
    ...
    z_promedio1.value = ''; z_desviacion1.value = ''; z_cantidad1.value = ''; z_promedio2.value = ''; z_desviacion2.value = ''; z_cantidad2.value = '';
...
const calcularZ = async () => {
    ...
    try {
        ...
        if (z_esDoble.value) data = { ...data, promedio1: parseFloat(z_promedio1.value), desviacion1: parseFloat(z_desviacion1.value), cantidad1: parseInt(z_cantidad1.value), promedio2: parseFloat(z_promedio2.value), desviacion2: parseFloat(z_desviacion2.value), cantidad2: parseInt(z_cantidad2.value) };
        else data = { ...data, promedio: parseFloat(z_promedio.value), u0: parseFloat(z_u0.value), desviacion: parseFloat(z_desviacion.value), cantidad: parseInt(z_cantidad.value) };
...
// =========================================================
//  2. LÓGICA TABLA 2.3 (PRUEBA T-STUDENT)
// =========================================================
const t_modoSeleccionado = ref<number | null>(null);
const t_mostrarResultados = ref(false);
const t_cargando = ref(false);
// Formulario T
const t_promedio = ref(''); const t_u0 = ref(''); const t_varianza = ref(''); const t_cantidad = ref(''); const t_confiabilidad = ref('');
...
const t_formularioValido = computed(() => {
  ...
  if (t_esDoble.value) return t_promedio1.value && t_varianza1.value && t_cantidad1.value && t_promedio2.value && t_varianza2.value && t_cantidad2.value && t_confiabilidad.value;
  else return t_promedio.value && t_u0.value && t_varianza.value && t_confiabilidad.value && t_cantidad.value;
});
...
const t_limpiarFormulario = () => {
    ...
    t_promedio1.value = ''; t_varianza1.value = ''; t_cantidad1.value = ''; t_promedio2.value = ''; t_varianza2.value = ''; t_cantidad2.value = '';
...
// =========================================================
//  3. LÓGICA PRUEBA DE PROPORCIÓN (NUEVO)
// =========================================================
const p_modoSeleccionado = ref<'two' | 'left' | 'right' | null>(null);
...
// Resultados
const p_z_stat = ref<number | null>(null);
const p_p_value = ref<number | null>(null);
const p_veredicto = ref('');

// Tarjetas de Selección visuales
const p_modos = [
    { id: 'two', titulo: 'H₀: p = p₀ vs H₁: p ≠ p₀', descripcion: 'Prueba Bilateral (Dos colas)', criterio: 'P-value < α' },
    { id: 'left', titulo: 'H₀: p ≥ p₀ vs H₁: p < p₀', descripcion: 'Prueba Unilateral Izquierda', criterio: 'P-value < α' },
    { id: 'right', titulo: 'H₀: p ≤ p₀ vs H₁: p > p₀', descripcion: 'Prueba Unilateral Derecha', criterio: 'P-value < α' }
];

const p_formularioValido = computed(() => {
    return p_modoSeleccionado.value && p_x.value && p_n.value && p_p0.value && p_confiabilidad.value;
});

const p_seleccionarModo = (id: string) => { 
    // @ts-ignore
    p_modoSeleccionado.value = id; 
    p_limpiarFormulario(); 
};

const p_limpiarFormulario = () => {
    p_x.value = ''; p_n.value = ''; p_p0.value = ''; p_confiabilidad.value = '';
    p_continuity.value = true;
    p_z_stat.value = null; p_p_value.value = null; p_veredicto.value = '';
    p_mostrarResultados.value = false;
};
...
const calcularProporcion = async () => {
    ...
    try {
        ...
        const payload = {
            x: Number(p_x.value),
            n: Number(p_n.value),
            p0: Number(p_p0.value),
            alpha: alphaCalculado,
            tail: p_modoSeleccionado.value,
            continuity: p_continuity.value ? 1 : 0
        };
        ...
        console.error(error);
        alert('Error al calcular proporción. Verifica los datos.');
    } finally {
...
<template>
  ...
  <div class="min-h-screen flex flex-col bg-gradient-to-br from-[#f8fafc] to-[#eef2f3] dark:from-[#0f0f0f] dark:to-[#1a1a1a] text-gray-800 dark:text-gray-100 transition-all p-4 sm:p-6">
    ...
    <main class="w-full max-w-5xl mx-auto space-y-6 flex-grow">
        ...
        <div class="bg-white/90 dark:bg-[#0b0b0b]/90 backdrop-blur rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden shadow-sm transition-all duration-300"
            ...
            <div v-show="activeAccordion === 'z-test'" class="border-t border-gray-100 dark:border-gray-800 p-6 animate-in slide-in-from-top-2">
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-indigo-100 dark:divide-gray-800">
                            <tr>
                                ...
                                <td rowspan="3" class="py-2 px-4 text-center border-l border-r border-indigo-100 dark:border-gray-800"><div class="flex flex-col items-center justify-center"><span class="text-xs font-bold bg-white dark:bg-gray-800 px-2 rounded mb-1 border">1 Muestra</span><div class="font-mono text-sm text-indigo-600 dark:text-indigo-400 font-bold">(x̄ - μ₀) / (σ / √n)</div></div></td>
                                <td class="py-2 px-4 text-center font-mono text-xs text-red-500">|Z₀| > Z(α/2)</td>
                            </tr>
                            <tr><td class="py-2 px-4"><div class="text-xs">H₀: μ ≥ μ₀ <br> H₁: μ < μ₀</div></td><td class="py-2 px-4 text-center font-mono text-xs text-red-500">Z₀ < -Zα</td></tr>
                            <tr><td class="py-2 px-4"><div class="text-xs">H₀: μ ≤ μ₀ <br> H₁: μ > μ₀</div></td><td class="py-2 px-4 text-center font-mono text-xs text-red-500">Z₀ > Zα</td></tr>
                            <tr class="bg-indigo-50/50 dark:bg-white/5"><td class="py-2 px-4"><div class="text-xs">H₀: μ₁ = μ₂ <br> H₁: μ₁ ≠ μ₂</div></td><td rowspan="3" class="py-2 px-4 text-center border-l border-r border-indigo-100 dark:border-gray-800"><div class="flex flex-col items-center justify-center"><span class="text-xs font-bold bg-white dark:bg-gray-800 px-2 rounded mb-1 border">2 Muestras</span><div class="font-mono text-sm text-indigo-600 dark:text-indigo-400 font-bold">(x̄₁ - x̄₂) / √(σ₁²/n₁ + σ₂²/n₂)</div></div></td><td class="py-2 px-4 text-center font-mono text-xs text-red-500">|Z₀| > Z(α/2)</td></tr>
                            <tr class="bg-indigo-50/50 dark:bg-white/5"><td class="py-2 px-4"><div class="text-xs">H₀: μ₁ ≥ μ₂ <br> H₁: μ₁ < μ₂</div></td><td class="py-2 px-4 text-center font-mono text-xs text-red-500">Z₀ < -Zα</td></tr>
                            <tr class="bg-indigo-50/50 dark:bg-white/5"><td class="py-2 px-4"><div class="text-xs">H₀: μ₁ ≤ μ₂ <br> H₁: μ₁ > μ₂</div></td><td class="py-2 px-4 text-center font-mono text-xs text-red-500">Z₀ > Zα</td></tr>
                        </tbody>
                    </table>
                </div>
                ...
                <div v-if="z_modoSeleccionado === null" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                     <button v-for="modo in z_modos" :key="modo.id" @click="z_seleccionarModo(modo.id)" class="p-4 rounded-lg bg-gray-50 dark:bg-[#151515] hover:bg-indigo-50 dark:hover:bg-indigo-900/20 border border-gray-200 dark:border-gray-700 hover:border-indigo-400 transition-all text-left">
                        <div class="flex justify-between items-center mb-2"><span class="font-bold text-indigo-600 dark:text-indigo-400">Caso {{ modo.id + 1 }}</span><span class="text-[10px] uppercase font-bold bg-white dark:bg-black px-2 py-1 rounded border border-gray-200 dark:border-gray-800">{{ modo.tipo === 'simple' ? '1 Muestra' : '2 Muestras' }}</span></div>
                        <p class="font-mono text-xs text-gray-800 dark:text-gray-200 mb-1">{{ modo.titulo }}</p><p class="text-xs text-gray-500">{{ modo.descripcion }}</p>
                     </button>
                </div>

                <div v-else class="space-y-6">
                    <div class="flex justify-between items-center bg-indigo-50 dark:bg-indigo-900/20 p-4 rounded-lg">
                        <div><h3 class="font-bold text-indigo-700 dark:text-indigo-300">{{ z_modoActual?.titulo }}</h3><p class="text-xs text-indigo-600/70 dark:text-indigo-400/70">Prueba Z - {{ z_modoActual?.descripcion }}</p></div>
        ...
        <div class="bg-white/90 dark:bg-[#0b0b0b]/90 backdrop-blur rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden shadow-sm transition-all duration-300"
            ...
            <div v-show="activeAccordion === 't-student'" class="border-t border-gray-100 dark:border-gray-800 p-6 animate-in slide-in-from-top-2">
                ...
                <div class="overflow-x-auto mb-8 rounded-lg border border-teal-100 dark:border-gray-800 bg-teal-50/30 dark:bg-black/20">
                    ...
                    <table class="w-full text-sm">
                        <thead><tr class="border-b border-teal-100 dark:border-gray-800 text-left bg-teal-50/50 dark:bg-white/5"><th class="py-2 px-4 font-semibold text-gray-600 dark:text-gray-400">Caso</th><th class="py-2 px-4 font-semibold text-center text-gray-600 dark:text-gray-400">Estadístico t₀</th><th class="py-2 px-4 font-semibold text-center text-gray-600 dark:text-gray-400">Grados Libertad (v)</th></tr></thead>
                        <tbody class="divide-y divide-teal-100 dark:divide-gray-800">
                            <tr><td class="py-2 px-4 font-medium">1 Muestra</td><td class="py-2 px-4 text-center font-mono text-teal-600 dark:text-teal-400 font-bold">(x̄ - μ₀) / (S / √n)</td><td class="py-2 px-4 text-center font-mono text-xs">n - 1</td></tr>
                            <tr><td class="py-2 px-4 font-medium">2 Muestras (Var. Iguales)</td><td class="py-2 px-4 text-center font-mono text-teal-600 dark:text-teal-400 font-bold">(x̄₁ - x̄₂) / (Sp · √...)</td><td class="py-2 px-4 text-center font-mono text-xs">n₁ + n₂ - 2</td></tr>
                            <tr><td class="py-2 px-4 font-medium">2 Muestras (Var. Diferentes)</td><td class="py-2 px-4 text-center font-mono text-teal-600 dark:text-teal-400 font-bold">(x̄₁ - x̄₂) / √(S₁²/n₁ + S₂²/n₂)</td><td class="py-2 px-4 text-center font-mono text-xs">Aprox. Welch</td></tr>
                        </tbody>
                    </table>
                </div>

                <div v-if="t_modoSeleccionado === null" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                     <button v-for="modo in t_modos" :key="modo.id" @click="t_seleccionarModo(modo.id)" class="p-4 rounded-lg bg-gray-50 dark:bg-[#151515] hover:bg-teal-50 dark:hover:bg-teal-900/20 border border-gray-200 dark:border-gray-700 hover:border-teal-400 transition-all text-left">
                        <div class="flex justify-between items-center mb-2"><span class="font-bold text-teal-600 dark:text-teal-400">Caso {{ modo.id + 1 }}</span><span class="text-[10px] uppercase font-bold bg-white dark:bg-black px-2 py-1 rounded border border-gray-200 dark:border-gray-800">{{ modo.tipo === 'simple' ? '1 Muestra' : '2 Muestras' }}</span></div>
                        <p class="font-mono text-xs text-gray-800 dark:text-gray-200 mb-1">{{ modo.titulo }}</p><p class="text-xs text-gray-500">{{ modo.descripcion }}</p>
                     </button>
                </div>
                ...
                <div v-else class="space-y-6">
                    <div class="flex justify-between items-center bg-teal-50 dark:bg-teal-900/20 p-4 rounded-lg">
                        <div><h3 class="font-bold text-teal-700 dark:text-teal-300">{{ t_modoActual?.titulo }}</h3><p class="text-xs text-teal-600/70 dark:text-teal-400/70">Prueba t - {{ t_modoActual?.descripcion }}</p></div>
                    ...
                    <form @submit.prevent="calcularT" class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        <div class="space-y-4">
                            ...
                            <div><label class="text-[10px] uppercase text-gray-500 font-bold">Confianza (1-α)</label><input v-model="t_confiabilidad" type="number" step="0.01" max="1" placeholder="0.95" required class="w-full px-3 py-2 rounded border dark:bg-[#151515] dark:border-gray-700 mt-1" /></div>
                            <Button type="submit" :disabled="!t_formularioValido || t_cargando" class="w-full bg-teal-600 hover:bg-teal-700 text-white">{{ t_cargando ? 'Calculando...' : 'Calcular t-Student' }}</Button>
                        </div>
        ...
        <div class="bg-white/90 dark:bg-[#0b0b0b]/90 backdrop-blur rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden shadow-sm transition-all duration-300"
             :class="activeAccordion === 'p-test' ? 'ring-2 ring-orange-500 shadow-xl' : 'hover:border-orange-300'">
            
            <button @click="toggleAccordion('p-test')" class="w-full flex items-center justify-between p-6 cursor-pointer focus:outline-none bg-gradient-to-r from-orange-50/50 to-transparent dark:from-orange-900/10">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-xl bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 flex items-center justify-center">
                        <PieChart class="w-6 h-6" />
                    </div>
                    <div class="text-left">
                        <h2 class="text-xl font-bold text-gray-900 dark:text-gray-100">Prueba de Proporción (Z)</h2>
                        <p class="text-sm text-gray-500 dark:text-gray-400">Aproximación Normal a la Binomial</p>
                    </div>
                </div>
                <div class="text-orange-500">
                    <ChevronUp v-if="activeAccordion === 'p-test'" class="w-6 h-6" />
                    <ChevronDown v-else class="w-6 h-6" />
                </div>
            </button>

            <div v-show="activeAccordion === 'p-test'" class="border-t border-gray-100 dark:border-gray-800 p-6 animate-in slide-in-from-top-2">
                
                <div class="overflow-x-auto mb-8 rounded-lg border border-orange-100 dark:border-gray-800 bg-orange-50/30 dark:bg-black/20">
                    <div class="bg-orange-100/50 dark:bg-gray-800/50 px-4 py-2 border-b border-orange-100 dark:border-gray-700">
                        <h3 class="text-xs font-bold uppercase tracking-wider text-orange-700 dark:text-gray-300">Fórmula de Referencia</h3>
                    </div>
                ...
                <div v-if="p_modoSeleccionado === null" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                     <button v-for="modo in p_modos" :key="modo.id" @click="p_seleccionarModo(modo.id)" 
                        class="p-4 rounded-lg bg-gray-50 dark:bg-[#151515] hover:bg-orange-50 dark:hover:bg-orange-900/20 border border-gray-200 dark:border-gray-700 hover:border-orange-400 transition-all text-left">
                        <div class="font-bold text-orange-600 dark:text-orange-400 mb-2">{{ modo.id === 'two' ? 'Bilateral' : (modo.id === 'left' ? 'Izquierda' : 'Derecha') }}</div>
                ...
                <div v-else class="space-y-6">
                    <div class="flex justify-between items-center bg-orange-50 dark:bg-orange-900/20 p-4 rounded-lg">
                        <div><h3 class="font-bold text-orange-700 dark:text-orange-300">Prueba {{ p_modoSeleccionado === 'two' ? 'Bilateral' : 'Unilateral' }}</h3><p class="text-xs text-orange-600/70 dark:text-orange-400/70">Proporción - Aproximación Normal</p></div>
                        <button @click="p_modoSeleccionado = null" class="text-xs font-bold text-orange-600 hover:underline">Cambiar Caso</button>
                    </div>

                    <form @submit.prevent="calcularProporcion" class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        <div class="space-y-4">
                            <div class="grid grid-cols-2 gap-3">
                                <div class="col-span-1"><label class="text-[10px] uppercase text-gray-500 font-bold">Éxitos (x)</label><input v-model="p_x" type="number" min="0" required class="w-full px-3 py-2 rounded border dark:bg-[#151515] dark:border-gray-700" /></div>
                                <div class="col-span-1"><label class="text-[10px] uppercase text-gray-500 font-bold">Muestra (n)</label><input v-model="p_n" type="number" min="1" required class="w-full px-3 py-2 rounded border dark:bg-[#151515] dark:border-gray-700" /></div>
                                <div class="col-span-2"><label class="text-[10px] uppercase text-gray-500 font-bold">Proporción Hipotética (p₀)</label><input v-model="p_p0" type="number" step="0.01" min="0" max="1" placeholder="Ej: 0.5" required class="w-full px-3 py-2 rounded border dark:bg-[#151515] dark:border-gray-700" /></div>
                            </div>
                            
                            <label class="flex items-center gap-2 p-3 bg-orange-50 dark:bg-orange-900/10 rounded border border-orange-100 dark:border-orange-800 cursor-pointer hover:bg-orange-100 dark:hover:bg-orange-900/30 transition">
                                <input type="checkbox" v-model="p_continuity" class="text-orange-600 rounded w-4 h-4 focus:ring-orange-500" />
                                <span class="text-sm font-semibold text-gray-700 dark:text-gray-300">Usar Corrección de Continuidad</span>
                            </label>

                            <div><label class="text-[10px] uppercase text-gray-500 font-bold">Confianza (1-α)</label><input v-model="p_confiabilidad" type="number" step="0.01" max="1" placeholder="0.95" required class="w-full px-3 py-2 rounded border dark:bg-[#151515] dark:border-gray-700 mt-1" /></div>
                            
                            <Button type="submit" :disabled="!p_formularioValido || p_cargando" class="w-full bg-orange-500 hover:bg-orange-600 text-white">
                        ...
                        <div class="bg-orange-50/50 dark:bg-orange-900/10 rounded-xl p-6 border border-orange-100 dark:border-orange-900/30 flex flex-col justify-center items-center text-center">
                            ...
                            <div v-else class="w-full space-y-4">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    ...
                                    <div class="bg-white dark:bg-black p-3 rounded border dark:border-gray-700">
                                        <p class="text-xs text-gray-500 uppercase">P-Value</p>
                                        <p class="text-xl font-mono font-bold">{{ p_p_value?.toFixed(4) }}</p>
                                    </div>
                                </div>
                                <button type="button" @click="p_limpiarFormulario" class="text-xs underline text-gray-500">Limpiar</button>
                            </div>
                        </div>
                    </form>
                </div>
